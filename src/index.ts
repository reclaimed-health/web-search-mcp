...
import { startGarbageCollector, GarbageCollector } from './garbage-collector.js';
...
class WebSearchMCPServer {
  ...
  constructor() {
    ...
    // Start garbage collector to automatically clean up orphaned Chrome processes
    this.garbageCollector = startGarbageCollector({
      intervalMs: 5 * 60 * 1000,      // Run every 5 minutes
      maxProcessAgeMs: 30 * 60 * 1000, // Kill Chrome processes older than 30 minutes
  ...
  private setupTools(): void {
    ...
    this.server.tool(
      ...
      'Search the web and fetch complete page content from top results. This is the most comprehensive web search tool. It searches the web and then follows the resulting links to extract their full page content, providing the most detailed and complete information available. Use get-web-search-summaries for a lightweight alternative.',
      {
        ...
        includeContent: z.any().optional(),
        maxContentLength: z.any().optional(),
      },
      async (args: any) => {
    ...
    this.server.tool(
      ...
      'Search the web and return only the search result snippets/descriptions without following links to extract full page content. This is a lightweight alternative to full-web-search for when you only need brief search results. For comprehensive information, use full-web-search instead.',
      {
        ...
        limit: z.any().describe('Number of search results to return (1-10)'),
      },
      async (args: unknown) => {
    ...
    this.server.tool(
      ...
      'Extract and return the full content from a single web page URL. This tool follows a provided URL and extracts the main page content. Useful for getting detailed content from a specific webpage without performing a search.',
      {
        ...
        maxContentLength: z.any().describe('Maximum characters for the extracted content (0 = no limit, undefined = use default limit). Usually not needed - content length is automatically optimized.'),
      },
      async (args: unknown) => {
  ...
  private validateAndConvertArgs(args: unknown): WebSearchToolInput {
    ...
    // Convert maxContentLength
    let maxContentLength: number | undefined;
    if (obj.maxContentLength !== undefined) {
      const val = typeof obj.maxContentLength === 'string' ? parseInt(obj.maxContentLength, 10) : obj.maxContentLength;
      if (typeof val === 'number' && !isNaN(val)) {
        maxContentLength = val;
      }
    }
  ...
  private setupGracefulShutdown(): void {
    ...
    process.on('SIGINT', async () => {
      ...
      try {
        ...
        // Emergency cleanup of any orphaned processes
        await this.garbageCollector.killAllPlaywrightChromes();
      } catch (error) {
